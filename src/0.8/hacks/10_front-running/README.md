# Front Running
#### Vulnerability
Transactions take some time before they are minted. An attacker can watch the transaction pool and send a transacstion to have it included in a block before the original transaction. This mechanism can be abused to re-order transactions to the attacker's advantage.
```
